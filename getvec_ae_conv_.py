...
# フォルダ作成
    ...
    with torch.no_grad():   # 重み更新をしない
        ...
        for (inputs, labels) in testloader:
            ...
            if cnt % 100 == 0:
                ...
                save_image(in_pic, './log/mlp_img/conv_{}_{}/input{}_{}_{}_image.png'.format(batch_size, learning_rate,cnt, batch_size, learning_rate))

                # テストデータに対する再構成画像を保存
                test_pic = to_img(outputs.cpu().data)
                save_image(test_pic, './log/mlp_img/conv_{}_{}/test{}_{}_{}_image.png'.format(batch_size, learning_rate, cnt, batch_size, learning_rate))
            """
            cnt += 1
    ...
    file = open('./log/loss_img/conv_test_loss/conv_test_loss_{}_{}.txt'.format(batch_size, learning_rate), 'w')
    file.write(text)
    file.close()
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.savefig("./log/loss_img/conv_loss{}_{}.png".format(batch_size, learning_rate))
    # }}}
    """
